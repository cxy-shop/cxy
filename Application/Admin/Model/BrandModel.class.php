...
class BrandModel extends XModel
{
    // 把表单字段映射到数据表的字段
    protected $_map = [
    ...
    protected $_validate = [
        ...
        ['cate_id', 'require', '名称必须！',self::MUST_VALIDATE],
        ['logo', 'require', 'logo必须！',self::EXISTS_VALIDATE ]
    ];
    //自动完成
    protected $_auto = [
        ['enable', 'toString', self::MODEL_BOTH, 'callback'], // 对name字段在新增和编辑的时候回调getName方法
        ['recommend', 'toString', self::MODEL_BOTH, 'callback'], // 对name字段在新增和编辑的时候回调getName方法
